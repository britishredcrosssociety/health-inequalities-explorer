...
wales_ltla_hi_outcomes <- wales_health_index |>
  ...
  add_row(number = mean(wales_health_index$healthy_people_score), area_name = "Wales Average") |>
...
wales_ltla_hi_risk_factors <- wales_health_index |>
  ...
  select(-ltla24_code) |>
  rename(area_name = ltla24_name) |>
  add_row(number = mean(wales_health_index$healthy_lives_score), area_name = "Wales Average") |>
  mutate(number = number + 100) |>
  mutate(rank = round(rank(number))) |>
  relocate(area_name) |>
  mutate(label = paste0(
...
wales_ltla_hi_social_determinants <- wales_health_index |>
  ...
  select(-ltla24_code) |>
  rename(area_name = ltla24_name) |>
  add_row(number = mean(wales_health_index$healthy_places_score), area_name = "Wales Average") |>
  mutate(number = number + 100) |>
  mutate(rank = round(rank(number))) |>
  relocate(area_name) |>
  mutate(label = paste0(
...
check_distribution(wales_ltla_hi_outcomes)
check_distribution(wales_ltla_hi_risk_factors)
check_distribution(wales_ltla_hi_social_determinants)

# ---- Save datasets ----
usethis::use_data(wales_ltla_hi_outcomes, overwrite = TRUE)
usethis::use_data(wales_ltla_hi_risk_factors, overwrite = TRUE)
usethis::use_data(wales_ltla_hi_social_determinants, overwrite = TRUE)
